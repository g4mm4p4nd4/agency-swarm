...
| `trace_id` | string | No | Explicit run trace identifier forwarded as `RunConfig.trace_id` for provenance continuity. |
...
| `client_config` | object | No | Override `base_url` / `api_key` / `default_headers` for this request (and optional `litellm_keys` for `litellm/` models). |
...
### How trace_id is applied
- If provided, FastAPI forwards `trace_id` into `RunConfig.trace_id`.
- This lets you pin a stable trace across retries and multi-step workflow hops.
- If omitted, Agency Swarm keeps its default trace behavior.
...
- `default_headers` (object, optional): Per-request OpenAI headers merged onto existing defaults.
  - Useful for routing and audit metadata such as `x-sandbox-id`, `x-user-id`, and `x-request-id`.
...
**Trace + sandbox metadata override:**

```json
{
  "message": "Continue this fleet run",
  "trace_id": "trace_fleet_001",
  "client_config": {
    "default_headers": {
      "x-sandbox-id": "sandbox-fleet-a",
      "x-request-id": "req-42"
    }
  }
}
